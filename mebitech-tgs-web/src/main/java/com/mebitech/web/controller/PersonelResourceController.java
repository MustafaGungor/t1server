...
@Controller
@RequestMapping("/resources/personels")
public class PersonelResourceController extends AController {
    private static Logger logger = LoggerFactory.getLogger(PersonelResourceController.class);
    @Autowired
    private IResponseFactory responseFactory;
    @Autowired
    private IPersonelRequestProcessor personelProcessor;
    @Autowired
    ...
    public Object list(HttpServletRequest request, HttpServletResponse response)
        ...
        IRestResponse restResponse = personelProcessor.findByFilters(filterAndPager);
    ...
    public Object add(@RequestBody String json) {
        try {
            PersonelImpl obj = objectMapper.readValue(json, PersonelImpl.class);
            obj.setCreateDate(new Date());
            return personelProcessor.add(obj).getData();
        } catch (Exception ex) {
    ...
    public Object update(@PathVariable("id") Long id,@RequestBody String json) {
        try {
            PersonelImpl obj = objectMapper.readValue(json, PersonelImpl.class);
            return personelProcessor.update(obj).getData();
        } catch (Exception ex) {
            return responseFactory.createResponse(RestResponseStatus.ERROR, "Hata : " + ex.getMessage());
        }
    }
    ...
    public IRestResponse delete(@PathVariable("id") Long id) {
        return personelProcessor.delete(id);
    }

    @RequestMapping(value = "/getProperties", method = RequestMethod.GET)
    @ResponseBody
    public IRestResponse getProperties() {
        return personelProcessor.getProperties();
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    @ResponseBody
    public IRestResponse findById(@PathVariable("id") Long id) {
        return personelProcessor.findById(id);
    }
