...
public class GroupController extends AController {
    ...
    @ResponseBody
    public Object addNewGroup(@RequestBody String json){
        try{
            GroupImpl group = objectMapper.readValue(json,GroupImpl.class);
            return groupRequestProcessor.add(group).getData();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @ResponseBody
    public Object updateGroup(@PathVariable("id") Long id,@RequestBody String json){
        try{
            GroupImpl group = objectMapper.readValue(json,GroupImpl.class);
            return groupRequestProcessor.update(group).getData();
        } catch (Exception e) {
            e.printStackTrace();
            return responseFactory.createResponse(RestResponseStatus.ERROR,"Hata : " + e.getMessage());
        }
    ...
    @ResponseBody
    public IRestResponse deleteGroup(@PathVariable("id") Long id){
        try{
            return groupRequestProcessor.delete(id);
        } catch (Exception e) {
            e.printStackTrace();
            return responseFactory.createResponse(RestResponseStatus.ERROR,"Hata : " + e.getMessage());
        }
    ...
    @ResponseBody
    public Object getAllGroups(HttpServletRequest request, HttpServletResponse response){
        FilterAndPagerImpl filterAndPager = getFilters(request);
        IRestResponse restResponse = groupRequestProcessor.findByFilters(filterAndPager);
